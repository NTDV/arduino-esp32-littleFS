...
void listDir(ntdv::FileSystem &fs, const char * dirname, uint8_t levels){
    Serial.printf("Listing directory: %s\r\n", dirname);

    ntdv::File root = fs.Open(dirname);
    if(!root){
    ...
    ntdv::File file = root.OpenNextFile();
    while(file){
        if(file.IsDirectory()){
            Serial.print("  DIR : ");
...
void writeFile(ntdv::FileSystem &fs, const char * path, const char * message){
    ...
    ntdv::File file = fs.Open(path, FILE_WRITE);
    if(!file){
...
void appendFile(ntdv::FileSystem &fs, const char * path, const char * message){
    ...
    ntdv::File file = fs.Open(path, FILE_APPEND);
    if(!file){
